...
import { createServer as createHttpServer } from 'http'; // 引入http模块中的createServer函数
import { createServer as createHttpsServer } from 'https'; // 引入https模块中的createServer函数
...
// 读取SSL证书和密钥
console.log('正在读取SSL证书和密钥...');
const httpsOptions = {
  key: fs.readFileSync(path.join(__dirname, './conf/check.znjz.online.key')),
  cert: fs.readFileSync(path.join(__dirname, './conf/check.znjz.online.pem')),
  requestCert: false,
  rejectUnauthorized: false  // 允许自签名证书
};
console.log('SSL证书和密钥读取成功');

// 创建HTTP和HTTPS服务器
const httpServer = createHttpServer(app); // 使用express应用创建一个http服务器
const httpsServer = createHttpsServer(httpsOptions, app); // 使用express应用和SSL选项创建一个https服务器

// 创建Socket.IO服务器，同时支持HTTP和HTTPS
const io = new Server({
  cors: {
    origin: "*", // 允许所有来源的跨域请求
...
console.log('正在初始化Socket.IO服务器...');
// 将Socket.IO服务器附加到HTTP和HTTPS服务器
...
io.on('connection', (socket) => { // 监听socket连接事件
  ...
  socket.on('join-room', (roomId: string) => { // 监听加入房间事件
    console.log(`用户 ${socket.id} 正在加入房间 ${roomId}`);
    ...
    console.log(`房间 ${roomId} 当前用户数: ${room.users.length}`);
  });

  // 处理 offer
  socket.on('offer', ({ offer, roomId, targetId }) => { // 监听offer事件
    console.log(`用户 ${socket.id} 向用户 ${targetId} 发送offer`);
  ...
  socket.on('answer', ({ answer, callerId }) => { // 监听answer事件
    console.log(`用户 ${socket.id} 向用户 ${callerId} 发送answer`);
  ...
  socket.on('ice-candidate', ({ candidate, targetId }) => { // 监听ICE candidate事件
    console.log(`用户 ${socket.id} 向用户 ${targetId} 发送ICE candidate`);
...
const HTTP_PORT = process.env.HTTP_PORT || 3001; // HTTP端口
const HTTPS_PORT = process.env.HTTPS_PORT || 3002; // HTTPS端口

// 启动HTTP服务器
httpServer.listen(HTTP_PORT, () => {
  console.log(`HTTP信令服务器运行在端口 ${HTTP_PORT}`);
});

// 启动HTTPS服务器
httpsServer.listen(HTTPS_PORT, () => {
  console.log(`HTTPS信令服务器运行在端口 ${HTTPS_PORT}`);
});